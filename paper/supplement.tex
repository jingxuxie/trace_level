...
\url{tracebreak/policies} & Local guard, visible-content DLP, visible-policy guard, TraceGuard, and StrictTraceGuard. \\
\url{tracebreak/experiments} & Deterministic scripted runner and bounded API runner. \\
\url{tracebreak/analysis} & Metric, paper-table, example-trace, and figure-generation scripts. \\
\url{results/raw_traces} & JSONL traces used to compute the reported tables. \\
\url{paper/tables} & Paper-facing tables, source-reference ablations, and compact example trace exports. \\
...
\midrule
DLP & 120 & 100 & 100 & 100 & 100 & 0 \\
Multi local & 120 & 100 & 100 & 100 & 100 & 0 \\
Single local & 120 & 100 & 100 & 100 & 100 & 0 \\
Visible policy & 120 & 83 & 100 & 67 & 67 & 33 \\
TraceGuard & 120 & 50 & 100 & 0 & 0 & 100 \\
\bottomrule
\end{tabular}
\caption{Deterministic 120-task matrix.}
\label{tab:det-supp-results}
\end{table}
...
\midrule
API DLP & 24 & 100 & 100 & 100 & 100 & 5.1 \\
API local & 24 & 100 & 100 & 100 & 100 & 5.1 \\
API policy prompt & 24 & 79 & 83 & 75 & 75 & 4.6 \\
API TraceGuard & 24 & 50 & 100 & 0 & 0 & 5.1 \\
\bottomrule
\end{tabular}
\caption{API subset with \texttt{gpt-4.1-mini}.}
\label{tab:api-supp-results}
\end{table}

\input{figures/api_security_utility.tex}

\input{tables/api_gpt41mini_24_by_category.tex}

\input{tables/traceguard_block_reason_audit.tex}

\input{tables/api_gpt41mini_policy_prompt_diagnostics.tex}

\input{tables/authority_transfer_deterministic_120.tex}

\input{tables/injection_overlay_deterministic_120.tex}

\input{tables/decoy_stress_deterministic_120.tex}

\input{tables/same_action_replay_gpt41mini_24.tex}

\input{tables/api_gpt41mini_visibility_gap_audit.tex}

\input{tables/api_gpt41mini_source_ref_compliance.tex}
...
\toprule
Source refs & Defense & Safe util. & Risky viol. & LPVR & Risky block & Safe FP \\
\midrule
Cooperative & TraceGuard & 100 & 0 & 0 & 100 & 0 \\
Deleted at sink & TraceGuard & 83 & 100 & 100 & 0 & 0 \\
Deleted at sink & Inferred TraceGuard & 100 & 0 & 0 & 100 & 0 \\
Deleted at sink & StrictTraceGuard & 8.3 & 0 & 0 & 100 & 92 \\
Corrupted refs & TraceGuard & 83 & 83 & 83 & 17 & 17 \\
Corrupted refs & Inferred TraceGuard & 100 & 0 & 0 & 100 & 0 \\
Corrupted refs & StrictTraceGuard & 83 & 83 & 83 & 17 & 17 \\
\bottomrule
\end{tabular}
\caption{Source-reference robustness ablation on the same 24
\texttt{gpt-4.1-mini} TraceGuard traces. Deleted rows remove sink
\texttt{source\_refs}; corrupted rows replace them with a benign prior public
reference. Inferred TraceGuard reconstructs the sink tag from prior
runtime-owned event tags. StrictTraceGuard blocks missing provenance but does
not address present-but-corrupted references.}
\label{tab:source-ref-ablation}
\end{table}
...
\toprule
Condition & Candidates & Repair success & Repair block & Risky viol. & Max steps \\
\midrule
API TraceGuard repair oracle & 12 & 100 & 0 & 0 & 1 \\
Scripted TraceGuard repair oracle & 60 & 100 & 0 & 0 & 2 \\
\bottomrule
\end{tabular}
\caption{Repair-oracle upper bounds over blocked risky TraceGuard traces. The
oracle appends matched safe-control continuations after the block. Scripted
aggregation-threshold repairs require one extra aggregation step before the safe
email, so this table shows that compliant continuations exist but does not
measure whether the language-model agent discovers them.}
\label{tab:repair-oracle}
\end{table}
...
The artifact also includes one aligned local/TraceGuard example for each policy
category in \texttt{paper/tables/api\_gpt41mini\_category\_examples.md}.
...
offsets, four conditions, and an eight-step maximum per task. For future
budgeted API sweeps, the runner records provider-reported token usage when it is
returned, and the metric script aggregates token totals by condition.
