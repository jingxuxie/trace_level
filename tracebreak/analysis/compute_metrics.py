        ...
        out.append(
            {
                ...
                "category": category,
                "risky_n": len(group),
                "global_violation_pct": pct(
                    sum(row["metrics"]["global_violation"] for row in group), len(group)
                ),
                "block_pct": pct(sum(row["metrics"]["blocked"] for row in group), len(group)),
            }
        )
    return out
...
def write_markdown(summary_rows: list[dict[str, Any]], category_rows: list[dict[str, Any]], path: str | Path) -> None:
    ...
    lines.extend(["", "## Risky Category Breakdown", ""])
